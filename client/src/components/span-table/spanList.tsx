...
const spanList = () => {
  ...
  const jsxElements = (() => {
    ...
    result.push([
      <div key="span-table-spacer-entry" className="span-entry-spacer">
        {' '}
      </div>,
    ]);
        ...
        <div key={entryKey} className="span-pod-entry">
          <div className="span-entry">
            <button
              ...
              onClick={() => {
                if (type === spanViewType.noRender) {
                  loadNewSpanResults(spanViewType.render, data[i]);
                } else {
                  loadNewSpanResults(spanViewType.noRender, data[i].spanIds);
                }
              }}
            >
              {'>'}
            </button>
            <div key={entryKey} className="span-name">
              <span className="boldSpanName span-label">SpanID: </span>{' '}
              {data[i].spanIds}
            </div>
          </div>
  ...
  return <div id="span-table-list">{jsxElements}</div>;
};
