...
export enum LoadingScreenType { percentage, cyclingStops }
...
/** 
   ...
   */
const loadingScreen = ({type = LoadingScreenType.cyclingStops}: LoadingScreenProps) => {
  if(type === LoadingScreenType.percentage) {
    const [progressPercent, setProgressPercent] = useState(0);
    
    useEffect(()=> {
        console.log(progressPercent);
        const updateProgress = () => setProgressPercent(Math.min(100, progressPercent + Math.max(0, Math.floor(Math.random() * 24))))
    ...
    const [currentProgress, setCurrentProgress] = useState('Loading');
      ...
      const updateProgress = () => setCurrentProgress(currentProgress.indexOf("...") === -1 ? currentProgress + '.' : currentProgress.replace('...', ''));
